...
# OpenAI (chat completions + image_url data-URL when a ready cover is present)
VIBE_CURATOR_PROVIDER=openai
OPENAI_API_KEY=...
OPENAI_MODEL=gpt-4o
VIBE_COVER_BASE_URL=http://api:3000

# Anthropic (Messages API + image blocks when a ready cover is present)
VIBE_CURATOR_PROVIDER=anthropic
ANTHROPIC_API_KEY=...
ANTHROPIC_MODEL=claude-sonnet-4-5
VIBE_COVER_BASE_URL=http://api:3000

# Ollama vision override (gemma4 is multimodal; set this only to swap the vision model)
# ollama pull gemma4
VIBE_CURATOR_PROVIDER=ollama
VIBE_OLLAMA_VISION_MODEL=
```
...
| `OPENAI_BASE_URL` | OpenAI chat-completions base (default `https://api.openai.com/v1`) |
| `OPENAI_MODEL` | OpenAI model (default `gpt-4o`; vision via `image_url` data-URL) |
| `ANTHROPIC_API_KEY` | Anthropic API key (`VIBE_ANTHROPIC_API_KEY` alias). Env fallback when owner UI key is unset. |
| `ANTHROPIC_BASE_URL` | Anthropic Messages API base (default `https://api.anthropic.com`) |
| `ANTHROPIC_MODEL` | Anthropic model (default `claude-sonnet-4-5`; vision via image blocks) |
...
**Design bind (extract then merge).** Archive-resident rows stay an **In archive** pill with Merge disabled and the existing “needs on-disk files” copy. Keep / Dismiss still work. The primary alt is **Extract & merge…**, which opens a confirm sheet: members listed as `pack.zip → path/foo.stl`, a target-model radio, copy *“Extracts selected files into the library folder (path-jailed), then merges. NFS files are not silent-deleted.”*, and **Extract & merge** / **Cancel**. Busy is **Extracting…** then the existing merge success (group `merged`). Errors stay rose inline; Keep / Dismiss remain available. Two-step **Extract…** is also confirmed HITL: after extract-only the new on-disk row is Loose / `mergeable: true` and the normal Merge confirm is used. Never silent, never a zip rewrite, never an NFS delete, never “Extract all” on the model archive panel, never “download whole zip to merge”.
...
| `openai` | `curator_runtime` / UI / `VIBE_CURATOR_PROVIDER=openai` | `POST {OPENAI_BASE_URL}/chat/completions` with `curator_runtime.openai_api_key` or `OPENAI_API_KEY`. Vision via `image_url` data-URL. Default model `gpt-4o`. |
| `anthropic` | `curator_runtime` / UI / `VIBE_CURATOR_PROVIDER=anthropic` | `POST {ANTHROPIC_BASE_URL}/v1/messages` with `x-api-key` from `curator_runtime.anthropic_api_key` or `ANTHROPIC_API_KEY`. Vision via image blocks. Default model `claude-sonnet-4-5`. |

**Vision (one image).** Live `ollama` / `xai` / `openai` / `anthropic` attach **at most one** cover to the chat user turn when a ranked model has `cover_status=ready` and a usable `cover_lqip_url` (else `cover_url`). Text catalog stays. Stub never loads or sends an image. Fetch/over-budget/missing → text-only fallback. No embeddings, no multi-view, no mesh bytes.
...
| `OPENAI_MODEL` | `gpt-4o` | OpenAI vision+text (`image_url` data-URL) |
| `ANTHROPIC_MODEL` | `claude-sonnet-4-5` | Anthropic vision+text (image blocks) |
| `VIBE_OLLAMA_MODEL` | `gemma4` | Owner UI / sidecar default (`ollama pull gemma4`). Multimodal; `VIBE_OLLAMA_VISION_MODEL` overrides only when an image is attached. |
