  ...
  return (
    <article className="min-w-0">
      <div className="relative overflow-hidden rounded-xl bg-ink-900">
        ...
        {onLike ? (
          <button
            ...
            className={`absolute right-2.5 top-2.5 rounded-full bg-ink-950/55 p-1.5 text-white backdrop-blur-sm disabled:opacity-60 ${
              model.liked ? "text-rose-300" : ""
            }`}
          >
      ...
      <div className="mt-2.5 min-w-0">
        <Link to={`/models/${model.id}`} className="block truncate font-display text-[15px] text-white">
          {model.title}
        </Link>
        {creator ? (
          <Link to={`/creators/${creator.slug}`} className="mt-0.5 block truncate text-sm text-slate-400">
            {creator.name}
          </Link>
        ) : null}
        ...
        {tag ? (
          onTag ? (
            <button
              type="button"
              onClick={() => onTag(tag)}
              className="mt-2 inline-flex rounded-full border border-accent-500/45 px-2 py-0.5 text-[11px] text-accent-400"
            >
              {tag}
            </button>
          ) : (
