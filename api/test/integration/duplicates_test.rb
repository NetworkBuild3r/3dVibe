...
class DuplicatesTest < ActionDispatch::IntegrationTest
  def setup
    ...
    @library = Library.create!(name: "Dup pile", root_path: @root.to_s, layout_mode: Library::LAYOUT_FLAT)
  ...
  test "extract copies an archive member onto disk then merge accepts the new asset" do
    ...
    assert_equal %w[assets extracted group merge model], response.parsed_body.keys.sort
  ...
  test "extract_and_merge streams the member then merges loose assets" do
    ...
    post "/api/v1/duplicates/#{group.id}/extract_and_merge",
         ...
         as: :json
    assert_response :created
    assert_equal %w[assets extracted group merge model review], response.parsed_body.keys.sort
